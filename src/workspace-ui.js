...
export function evidence(p) {
  const reviewed = verifiedCount(p) > 0;
  const main = p.id === 'oakridge' ? `${reviewed ? `<div class="review-success">${icon('check')}<div><strong>Final report unlocked</strong>${statusChip('2 changes verified', 'done')}<p>Illustrative index: 82 → 58. Wider vegetation and roof/vent details remain unresolved.</p></div></div>` : `<div class="verification-path"><div class="panel-intro"><div><h3>${p.prepared ? 'Step 2: review documented changes' : 'Get your final report'}</h3><p>${p.prepared ? 'Confirm exactly what sample evidence supports. Then report unlocks.' : 'For this demo, load matching before/after evidence. Nothing else to fill in.'}</p></div></div><ol><li class="${p.prepared ? 'complete' : 'current'}"><span>${p.prepared ? icon('check') : '1'}</span><div><strong>Load prepared evidence</strong><small>Matching synthetic before/after images for two completed tasks.</small></div></li><li class="${p.prepared ? 'current' : ''}"><span>2</span><div><strong>Review two observable changes</strong><small>Score stays at 82 until review is applied.</small></div></li><li><span>3</span><div><strong>Open final report</strong><small>Then owner reviews insurer packet.</small></div></li></ol></div>`}${comparison()}
    ${reviewed ? footer('See updated risk','open-updated-risk','Next: review what changed before building report.') : p.prepared ? footer('Review 2 changes and unlock report','review-prepared','You will see exactly what changes count before anything updates.') : footer('1. Load prepared evidence','load-prepared','One click. Then review two documented changes.')}` : `<div class="panel-intro"><div><h3>Document your progress</h3><p>Add photos or documents for review.</p></div></div>${button('Open Oakridge scenario','select-oakridge','secondary','spark')}`;
  return `${main}
