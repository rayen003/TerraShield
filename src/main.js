...
import { createInitialState, exposureCategory, evidenceCoverage, summary, unresolved, verifiedCount, pendingReview, loadPrepared, applyReview, reportSummary, BASELINE_DATE, FOLLOWUP_DATE, ASSESSMENT_NOTICE, scheduleTemplate, useRecommendedPlan, startWork, markOwnerComplete, priorityActions, refreshWorkflow, requestCorrection } from './data.js';
...
const riskBadge = p => badge(`${exposureCategory(p)} exposure`, exposureCategory(p).toLowerCase());
...
function portfolio() {
  ...
  return `<main><div class="page-heading"><div><div class="breadcrumb">TerraShield <span>/</span> Portfolio</div><h1>A wider view. The same attention to detail.</h1><p>Portfolio view illustrates future insurer and property-manager workflows.</p></div>${btn('Ask TerraShield','assistant','assistant-trigger','spark')}</div><div class="summary-grid">${[[s.total,'Properties monitored','home'],[s.elevated,'Elevated surrounding exposure','alert'],[s.pending,'Awaiting evidence review','camera'],[s.verified,'With verified actions','shield']].map(([n,label,ic]) => `<div class="summary-card"><span>${label}</span>${icon(ic)}<strong>${n.toString().padStart(2,'0')}</strong></div>`).join('')}</div><div class="portfolio-map">${mapMarkup(true)}</div><section class="portfolio-table"><div class="table-toolbar"><div><h2>Property overview</h2><p>Five fictional properties. One shared workspace.</p></div><div class="filters"><label class="search-field">${icon('search')}<input id="property-search" type="search" placeholder="Search properties" value="${e(search)}" aria-label="Search properties"></label><select id="workflow-filter" aria-label="Filter workflow">${[['all','All properties'],['action','Action needed'],['pending','Evidence pending'],['verified','Verified actions present'],['gaps','Outstanding evidence gaps']].map(([id,label]) => `<option value="${id}" ${id === filter ? 'selected' : ''}>${label}</option>`).join('')}</select><select id="risk-filter" aria-label="Filter surrounding exposure">${['all','Elevated','Moderate','Lower'].map(c => `<option value="${c}" ${c === riskFilter ? 'selected' : ''}>${c === 'all' ? 'All exposure categories' : c}</option>`).join('')}</select></div></div><div id="table-results">${tableRows()}</div></section></main>`;
}
function tableRows() {
  const rows = state.properties.filter(p => `${p.name} ${p.type}`.toLowerCase().includes(search.toLowerCase()) && (riskFilter === 'all' || exposureCategory(p) === riskFilter) && (filter === 'all' || filter === 'action' && p.workflow === 'Action needed' || filter === 'pending' && pendingReview(p) || filter === 'verified' && verifiedCount(p) > 0 || filter === 'gaps' && evidenceCoverage(p).label !== 'Sufficient for the stated assessment scope'));
  return `<div class="table-scroll"><table><thead><tr><th>Property</th><th>Property type</th><th>Surrounding exposure</th><th>Open observed concerns</th><th>Actions verified</th><th>Evidence coverage</th><th>Latest assessment</th><th><span class="sr-only">Open property</span></th></tr></thead><tbody>${rows.map(p => `<tr><td><strong>${e(p.name)}</strong><small>Sonoma County, CA</small></td><td>${e(p.type)}</td><td>${riskBadge(p)}</td><td>${p.observations.filter(o => o.findingStatus === 'Observed concern').length}</td><td><span class="verified-cell">${icon(verifiedCount(p) ? 'check' : 'clock')}${verifiedCount(p)} of ${p.actions.length}</span></td><td>${e(evidenceCoverage(p).label)}</td><td>${date(p.assessmentDate)}</td><td>${btn('Open property',`select-${p.id}`,'text-button','arrow')}</td></tr>`).join('')}</tbody></table></div>${rows.length ? `<p class="table-count">${rows.length} of 5 properties · Exposure categories are illustrative</p>` : `<div class="empty-state"><h3>No matching properties</h3><p>Try a different search or clear your filters.</p>${btn('Clear filters','clear-filters','secondary')}</div>`}`;
}
function reports() {
...
function report(p) {
  ...
  const r = reportSummary(p), coverage = evidenceCoverage(p), exposure = p.exposureAssessment;
  return `<main class="report-main"><div class="report-toolbar">${btn('Return to property','return-property','text-button','home')}<div>${btn('Download JSON','download-report','secondary','download')}${btn('Print / Save as PDF','print-report','primary','print')}</div></div><article class="report-document"><div class="report-brand"><span class="brand">${icon('shield')}TerraShield</span>${badge('Illustrative demonstration output','demo')}</div><div class="report-title"><div><p class="muted">Assessment and mitigation review</p><h1>${e(p.name)}</h1><p>Fictional property record - ${e(p.location)}</p></div></div><div class="report-dates"><div><span>Baseline assessment</span><strong>${date(BASELINE_DATE)}</strong></div><div><span>Follow-up evidence</span><strong>${date(FOLLOWUP_DATE)}</strong></div><div><span>Methodology</span><strong>${e(r.methodologyVersion)}</strong></div></div><section class="report-assessment"><h2>Surrounding wildfire exposure</h2><strong>${e(exposure.category)} - Illustrative exposure category</strong><p>${e(exposure.geographicScope)}</p><p>Basis: ${e(exposure.drivers.join('; '))}</p><p>Limitations: ${e(exposure.limitations)}</p></section><section class="report-assessment"><h2>Mitigation and evidence</h2><p><strong>${verifiedCount(p)} of ${p.actions.length} recommended actions verified in demo.</strong> Action completion does not represent a reduction in wildfire probability or expected loss.</p><p><strong>Evidence coverage: ${coverage.label}</strong> - ${e(coverage.scope)}</p><p>Outstanding: ${e(coverage.outstanding.join('; '))}</p></section><h2>Observed property findings</h2>${p.observations.map(o => `<div class="report-outstanding"><span>${icon(o.findingStatus === 'Change verified' ? 'check' : 'clock')}</span><div><strong>${e(o.title)}</strong><p>${e(o.propertyArea)} - ${e(o.findingStatus)} - ${date(o.observedAt)}</p><p>${e(o.source)}</p></div></div>`).join('')}<h2>Review method</h2><p>Simulated human operator review using prepared demonstration evidence. Verification confirms only specific observable changes in reviewed evidence.</p><p class="report-definition">${ASSESSMENT_NOTICE}</p><footer class="report-disclaimer">${e(r.closingNote)} An assessment is not a guarantee that a property will or will not be affected by wildfire. TerraShield is not an emergency warning or evacuation service; follow official emergency instructions.</footer></article></main>`;
}
...
function correctionModal(findingId) {
  const finding = current().observations.find(o => o.id === findingId); if (!finding) return;
  modal('Flag an assessment issue', `<p><strong>${e(finding.title)}</strong></p><form data-form="correction" data-finding="${e(findingId)}" class="insurance-form"><label>What needs review?<textarea name="note" rows="4" maxlength="500" required placeholder="Describe the issue or missing context…"></textarea></label><p class="form-hint">Recorded locally for demonstration; not sent to a review team. The finding remains visible until review.</p><button class="button primary full" type="submit">Request review</button></form>`);
}
...
app.addEventListener('click', event => {
  ...
  if (action.startsWith('flag-')) { correctionModal(action.slice(5)); return; }
  ...
  if (action.startsWith('assistant-')) {
    ...
    else openTab({ plan: 'plan', overview: 'overview', 'updated-risk': 'updated-risk', report: 'report', evidence: 'evidence' }[sub] || 'overview');
    return;
  }
  switch(action) {
    ...
    case 'assessment-info': modal('How to read this assessment', `<div class="assessment-help"><h3>Exposure</h3><p>What could affect this location? It describes surrounding conditions, not building resistance.</p><h3>Vulnerability</h3><p>What observed property conditions could contribute to damage?</p><h3>Mitigation</h3><p>What recommended work has been documented and verified?</p><h3>Evidence coverage</h3><p>What do we know, and what remains uncertain?</p><p>${ASSESSMENT_NOTICE}</p><p>An assessment is not a guarantee that a property will or will not be affected by wildfire. Verification confirms specific observable changes only. Insurance pricing, eligibility and coverage are determined by the insurer. TerraShield is not an emergency warning or evacuation service; follow official emergency instructions.</p></div>`); break;
    ...
    case 'reset': if (state.dirty) modal('Reset this demo?', '<p>This restores all five original property records, original plans and evidence, and an empty assistant conversation. Local uploads will be cleared.</p>', `${btn('Keep exploring','close-modal','secondary')}${btn('Reset demo','confirm-reset','primary','reset')}`); else resetDemo(); break;
...
app.addEventListener('submit', event => {
  ...
  switch(form.dataset.form) {
    ...
    case 'correction': { if (requestCorrection(p, form.dataset.finding, data.get('note'))) { save('Review requested. Recorded locally for demonstration.'); document.getElementById('modal').close(); render(); } break; }
